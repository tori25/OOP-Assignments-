// This class extends Board and is optimized for the DotBox game.
class DotBoxBoard extends Board {
    ...
    public int claimEdge(int r, int c, char dir, int playerIndex) {
        ...
        if (dir == 'H') {
            ...
            if (r > 0 && isBoxComplete(r - 1, c)) {
                if (boxOwner[r - 1][c] == -1) { boxOwner[r - 1][c] = playerIndex; completed++; }
            }
            // box below (r, c)
            if (r < rows && isBoxComplete(r, c)) {
                if (boxOwner[r][c] == -1) { boxOwner[r][c] = playerIndex; completed++; }
            }
            return completed;
        } else if (dir == 'V') {
            if (r < 0 || r >= rows || c < 0 || c > cols) return -1;
            ...
            if (c > 0 && isBoxComplete(r, c - 1)) {
                if (boxOwner[r][c - 1] == -1) { boxOwner[r][c - 1] = playerIndex; completed++; }
            }
